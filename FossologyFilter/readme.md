...
通过一些规则，对Fossology扫描结果中“不合理”的部分进行过滤

## 使用方法

可以参考[example.py](../example.py)里的内容

在使用Fossology扫描一个文件后，会得到多个扫描结果，结果中包含了`许可证名称`、`判断依据文本`、`文本在原文中的位置`等信息

将这些信息输入到过滤工具中后自动对结果进行过滤，仅返回`合理`的扫描结果
